/-
  ...
  PartB/SignIff.lean — Sign of Bell asymmetry decides LLPO

  Under AtMostOne:
    bellAsymmetry α ≤ 0  →  ∀ n, α(2n) = false
    0 ≤ bellAsymmetry α  →  ∀ n, α(2n+1) = false
...
/-- Under AtMostOne, if bellAsymmetry α ≤ 0, then all even entries are false.

    Proof: If some α(2k) = true, then evenField > 0 (by tsum_pos) and
    AtMostOne forces all odd entries false, so oddField = 0. Then
    bellAsymmetry = evenField - 0 > 0, contradicting ≤ 0. -/
theorem bellAsymmetry_nonpos_implies_even_false (α : ℕ → Bool)
    (hamo : AtMostOne α) (hle : bellAsymmetry α ≤ 0) :
    ∀ n, α (2 * n) = false := by
  intro n
  by_contra hne
  push_neg at hne
...
/-- Under AtMostOne, if 0 ≤ bellAsymmetry α, then all odd entries are false.
    ...
    bellAsymmetry = 0 - oddField < 0, contradicting ≥ 0. -/
theorem bellAsymmetry_nonneg_implies_odd_false (α : ℕ → Bool)
    (hamo : AtMostOne α) (hge : 0 ≤ bellAsymmetry α) :
    ∀ n, α (2 * n + 1) = false := by
  intro n
  by_contra hne
  push_neg at hne
