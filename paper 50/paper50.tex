...
\]
Here $\LPO$ (Limited Principle of Omniscience) states that every binary sequence is identically zero or contains a~$1$. In field-theoretic form, $\LPO(K)$ states $\forall x \in K,\; x = 0 \lor x \neq 0$. Over the Archimedean completion~$\R$, exact zero-testing of a Cauchy sequence requires $\LPO$: the Archimedean metric provides no finite decidability radius, so confirming $x = 0$ requires inspecting all approximants. Over a non-Archimedean completion~$\Qp$, the ultrametric inequality $|a + b|_p \leq \max(|a|_p, |b|_p)$ makes the topology totally disconnected, and for pure motives the essential spectral data (Frobenius traces) descend to~$\Z$ by Deligne's Weil~II theorem; thus the relevant computations are $\BISH$-decidable. Over $\Q$, $\Qbar$, or $\Z$, equality is decidable by finite computation in $\BISH$. $\MP$ (Markov's Principle) asserts $\lnot\lnot P \to P$ for decidable $P$; it governs unbounded search through discrete sets. For a thorough treatment of $\CRM$, see Bridges--Richman~\cite{BridgesRichman1987}.
...
\begin{remark}[The algebraic layers of $\R$]\label{rmk:R-layers}
The $u$-invariant $u(\R) = \infty$ is a property of $\R$-as-ordered-field: positive-definiteness requires the ordering~$<$ but not the completeness (least upper bound property). This distinction matters because $\R$ carries three nested algebraic structures with increasing CRM cost:
\begin{enumerate}[label=(\roman*)]
\item $(\R, +, \times)$ as a ring: polynomial arithmetic, decidable by \texttt{ring}. Cost: $\BISH$.
\item $(\R, +, \times, <)$ as an ordered field: sign decisions, zero-testing, positive-definiteness. Cost: $\LPO$ (or $\WLPO$ for equality testing alone).
\item $(\R, +, \times, <, \mathrm{LUB})$ as a complete ordered field: integration, spectral decomposition, existence of limits. Cost: $\CLASS$.
\end{enumerate}
The three DPT axioms interact with different layers: Axiom~1 (decidable morphisms) and Axiom~2 (algebraic spectrum) operate at the ring layer ($\BISH$); Axiom~3 (Archimedean polarization) operates at the ordered field layer, exploiting $u(\R) = \infty$. The comparison isomorphisms that the DPT axioms bypass---period maps, the Hodge decomposition---operate at the completeness layer ($\CLASS$). This stratification, implicit in the present paper, is made explicit in Paper~98.
\end{remark}
