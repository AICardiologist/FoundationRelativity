...
\subsection*{Interlude: Noether's theorem (1918)}

In the same year that Weyl published \emph{Das Kontinuum}, Emmy Noether proved the theorem that bears her name.
She had been invited to G\"ottingen by Klein and Hilbert specifically to resolve a puzzle about energy conservation in Einstein's general relativity: the field equations seemed to violate the conservation laws that held for every other physical theory.
Noether's resolution was a masterpiece of algebraic reasoning: every continuous symmetry of the action functional yields a conserved current.
Time-translation invariance gives energy conservation; spatial translation gives momentum; rotational invariance gives angular momentum.
The theorem is variational calculus on finite parameter spaces---algebraic manipulation of Lagrangians, derivatives, and symmetry transformations.
It is BISH to the core.

What happened next follows the pattern of Act~I exactly.
Noether's theorem was immediately elevated from a \emph{local} statement (the continuity equation $\partial_\mu j^\mu = 0$ holds at every point) to a \emph{global} one (the total conserved charge $Q = \int j^0\,d^3x$ is a constant of the motion for all time).
The local version---continuity equations on finite lattices, energy-momentum balance over finite grids---is BISH: finite sums, finite differences, algebraic identities.
This is precisely what Paper~15 formalizes \cite{Lee2026k}.
The global version asserts that partial sums of nonnegative densities converge to a definite total as the summation extends over infinitely many spatial sites and time steps.
This is Noether partial-sum convergence (NPSC), equivalent to bounded monotone convergence---the same BMC~$\equiv$~LPO that governs the thermodynamic limit and geodesic incompleteness.

The historical irony is sharp.
Conservation laws are often presented as the most fundamental principles in physics, more basic even than the equations of motion.
Yet the ``law'' of energy conservation---the global assertion that the total energy of an infinite system is constant forever---is an LPO idealization.
What every simulation computes, what every calorimeter measures, what every engineering specification relies on, is the \emph{local} version: energy balance over finite regions and finite times.
The cellar held the physics all along; the law was the cathedral.

But the story has a darker sequel.
Noether's 1918 paper actually contained \emph{two} theorems.
The first---the famous one---applies when the symmetry group is finite-dimensional (time translation, spatial rotation) and yields genuine conservation laws.
The second, less cited but equally important, applies when the symmetry group is infinite-dimensional, as with the diffeomorphism invariance of general relativity.
In that case, the ``conserved quantities'' are either trivially zero (the contracted Bianchi identity, which holds as an algebraic identity, not a dynamical law) or reduce to coordinate-dependent boundary terms.
This was precisely the puzzle Klein and Hilbert had asked her to solve---and her answer was essentially negative: general covariance means that GR does not yield proper energy conservation laws via the standard variational route.
A century of subsequent work has confirmed her verdict.
The Einstein and Landau-Lifshitz pseudotensors are coordinate-dependent---not tensors at all---and can be made to give any value, including zero, at any point by an appropriate choice of coordinates.
The ADM and Bondi energies are well-defined only for asymptotically flat spacetimes, which the actual universe is not.
In an expanding FLRW cosmology, photons lose energy to the cosmological redshift, and no well-defined gravitational field energy compensates for the loss; the time-translation symmetry that Noether's first theorem requires simply does not exist.

The LPO step thus buys not merely an idealization but a quantity that is physically ill-defined.
To obtain a global conservation law, one must integrate a local energy density over all of space and assert convergence of the infinite sum---an LPO operation.
In general relativity, the integrand itself is ambiguous (gravitational energy cannot be localized), the integration domain is curved and dynamical (the choice of spatial hypersurface matters), and in cosmological settings the requisite symmetry does not exist.
The completed-infinity limit that LPO purchases corresponds to a quantity that a century of effort has failed to define unambiguously.
This is the strongest possible case that the LPO expenditure is not an innocent mathematical abstraction: it is an idealization of something the physics itself cannot coherently supply.
...
\subsection*{Interlude: the measurement problem and decoherence}

Von~Neumann's 1932 axiomatization introduced not only infinite-dimensional Hilbert spaces but also a foundational puzzle that remains unresolved.
He distinguished two processes: Process~2 (Schr\"odinger evolution), which is continuous, deterministic, and unitary; and Process~1 (measurement), which is discontinuous, probabilistic, and projects the state onto an eigenstate of the measured observable.
The measurement problem asks: when does Process~1 apply instead of Process~2?
Von~Neumann placed the boundary between the two processes at the observer's consciousness---a solution he himself found unsatisfying, and which the physics community has debated for nine decades through Copenhagen, many-worlds, GRW collapse, and other interpretations.

The decoherence programme, initiated by Zeh (1970) and developed by Zurek (1981--2003), offered a partial resolution.
When a quantum system interacts with its environment---air molecules, photons, thermal radiation---the off-diagonal elements of the system's density matrix decay exponentially.
The system \emph{appears} to collapse into a definite state, not because of a separate physical process, but because the quantum coherence has leaked into environmental degrees of freedom that the observer does not track.
This is the mechanism that Paper~14 formalizes \cite{Lee2026j}.

The constructive calibration reveals the now-familiar pattern.
Decoherence bounds---the off-diagonal decay estimate $|\rho_{01}(t)| \leq e^{-\gamma t}$ for a qubit coupled to an $N$-mode environment---are pure finite-dimensional linear algebra: partial traces, matrix norms, geometric series.
This is BISH \cite{Lee2026j}.
The Born-rule probabilities $p_k = |\langle \phi_k | \psi \rangle|^2$, computed from finite-dimensional inner products, are likewise BISH \cite{Lee2026l}.
The frequentist assertion that these probabilities emerge as limiting relative frequencies over infinitely many measurements requires Dependent Choice for the sequential construction of the frequency limit \cite{Lee2026l}.
And \emph{exact} decoherence---the assertion that off-diagonal elements converge to precisely zero as the environment grows without bound---requires BMC~$\equiv$~LPO, the same bounded monotone convergence that governs the thermodynamic limit, geodesic incompleteness, and global energy conservation \cite{Lee2026j}.

The measurement problem, viewed through the constructive lens, has the same cellar-and-cathedral structure as every other topic in this essay.
What experiments verify---finite decoherence bounds, Born probabilities for specific measurements---is BISH\@.
What the interpretive debate concerns---exact collapse, exact decoherence, the emergence of definite outcomes in infinite environments---involves completed limits at LPO or sequential constructions at DC$_\omega$.
The operational content of quantum measurement sits at sea level.
The ontological questions that have generated ninety years of philosophy ascend into the cathedral.
