...
\subsection{The Taylor--Wiles audit}

The de-omniscientizing descent (\S\ref{sec:descent}) classifies
the logical content of the five major conjectures.  The natural
next step is to classify the logical content of the
\emph{proofs}---specifically, the Taylor--Wiles patching argument
that underlies all modern modularity lifting theorems.

The question is: what is the omniscience cost of the
Taylor--Wiles method?  A preliminary analysis suggests three
sites where non-constructive content may enter:
\begin{enumerate}
\item The patching argument constructs a limit over an inverse
  system of Hecke algebras (compactness---classical).
\item Galois representations via the Chebotarev density theorem
  (deciding splitting conditions---$\LPO$).
\item The $R = T$ theorem uses Nakayama's lemma in a completed
  local ring (constructively valid if the ring is explicitly
  presented) and a numerical criterion involving Selmer groups
  and $L$-values.
\end{enumerate}

A systematic CRM audit of Wiles's original argument for
semistable elliptic curves---listing every non-constructive step
and classifying it as $\MP$, $\LPO$, or eliminable---would
determine whether the patching method is essentially $\LPO$ or
contains eliminable non-constructive content.  This is deferred
to future work.

\subsection{The synthesis monograph}

This paper is itself a step toward a standalone monograph
packaging the program.  The full monograph would integrate
the physics calibrations (Papers~1--44, synthesized in
Paper~40), the arithmetic geometry calibrations (this paper),
and the formal verification corpus into a single document.
...
The Constructive Reverse Mathematics program, across sixty-six
...
The thesis has held up.  The five great conjectures exhibit a
...
the full mixed motive frontier.  At the Axiom~1 boundary, where
...
the form-class invariant, the three-invariant classification---that
