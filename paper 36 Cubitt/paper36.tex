...
\title{%
  ...
  {\normalsize Paper~36 of the Constructive Reverse Mathematics Series}\\[4pt]
...
(Paper~29~\cite{Lee26P29}), phase transitions (Paper~29),
and gauge coupling existence (Paper~32~\cite{Lee26P32}),
Cubitt's undecidability introduces
...
for Weihrauch reducibility as a framework, see
Brattka--Gherardi--Pauly~\cite{brattka2021}.
Paper~29~\cite{Lee26P29} proved $\BMC \iff \LPO$.
Paper~35~\cite{Lee26P35} established the
...
theorem pointwise_gap_decidable
    (M : TM) (hl : LPO) :
    spectral_gap M > 0 $\lor$ spectral_gap M = 0 := by
  rcases hl (halting_seq M) with
    h_all_false | <<n, hn>>
  $\cdot$ left
    exact cpgw_gapped_of_not_halts M _
  $\cdot$ right
...
$\Delta = 0 \lor \Delta \neq 0$ is equivalent to $\WLPO$.
\end{theorem}

\begin{remark}[WLPO vs.\ LPO for zero-tests]\label{rem:wlpo-lpo}
The constructive content of this theorem hinges on the
\emph{negative} second disjunct. Deciding
$\Delta = 0 \lor \Delta > 0$ (positive disjunct) for non-negative
reals is equivalent to $\LPO$, not $\WLPO$: given a binary
sequence~$\alpha$, define $\Delta_\alpha = \sum \alpha(n) \cdot 2^{-n}$.
Then $\Delta_\alpha = 0$ iff $\forall n,\, \alpha(n) = 0$, and
$\Delta_\alpha > 0$ iff $\exists n,\, \alpha(n) = 1$---so deciding
$\Delta_\alpha = 0 \lor \Delta_\alpha > 0$ solves $\LPO$ directly.

$\WLPO$'s signature is the negative formulation:
$\Delta = 0 \lor \Delta \neq 0$, where $\Delta \neq 0$ means
$\neg(\Delta = 0)$. For a non-negative real, $\Delta \neq 0$ does
\emph{not} constructively imply $\Delta > 0$. Upgrading
$\Delta \neq 0$ to $\Delta > 0$ requires Markov's Principle
($\MP$), and it is a known theorem that
$\WLPO + \MP \iff \LPO$~\cite{ishihara2006, bridgesvita2006}.
This distinction is invisible in classical mathematics but is
precisely the gap between $\WLPO$ and $\LPO$ in the constructive
hierarchy.
\end{remark}

This connects to Paper~26~\cite{Lee26P26} (the physical
instantiation of the G\"odel--Gap embedding) and
Paper~33~\cite{Lee26P33} (the QCD mass gap is a
...
theorem cubitt_is_lpo :
    (forall (M : TM),
      spectral_gap M > 0 $\lor$ spectral_gap M = 0)
    <-> LPO :=
...
$\WLPO$ & ``$\Delta = 0$ or $\Delta \neq 0$?'' & Zero-test on completed real (negative disjunct) \\
...
Paper~26~\cite{Lee26P26} proved that $\Pi^0_1$ arithmetic sentences
order-embed into $\ell^\infty / c_0$, with $\WLPO$ as the logical
barrier. Cubitt's construction is the physical realization:
...
Cubitt's undecidability is not ``G\"odelian'' (self-referential
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Source}: \texttt{P36\_CubittStratification/} (8 files, 667 lines)
...
$\WLPO$ (with the crucial caveat that the negative disjunct
$\Delta \neq 0$ is used, not the positive $\Delta > 0$; see
Remark~\ref{rem:wlpo-lpo}), and the uniform function is
$\LPO$-computable but non-computable without $\LPO$. Since $\LPO$
is already required for thermodynamic limits, phase transitions,
and gauge couplings, macroscopic quantum undecidability costs
exactly one thermodynamic limit---and introduces nothing new.
...
\bibitem{Lee26P26}
P.~C.-K.~Lee.
\newblock The G\"odel--Gap embedding: $\Pi^0_1$ sentences in
  $\ell^\infty / c_0$ and the $\WLPO$ barrier.
\newblock Preprint, 2026. Paper~26.

\bibitem{Lee26P29}
P.~C.-K.~Lee.
\newblock Bounded monotone convergence is $\LPO$: the logical cost
  of thermodynamic limits.
\newblock Preprint, 2026. Paper~29.

\bibitem{Lee26P32}
P.~C.-K.~Lee.
\newblock The running coupling existence problem is $\LPO$.
\newblock Preprint, 2026. Paper~32.

\bibitem{Lee26P33}
P.~C.-K.~Lee.
\newblock The QCD mass gap is a $\WLPO$ zero-test.
\newblock Preprint, 2026. Paper~33.

\bibitem{Lee26P35}
P.~C.-K.~Lee.
\newblock The conservation metatheorem: physics lives at $\BISH + \LPO$.
\newblock Preprint, 2026. Paper~35.
...
\bibitem[Brattka et~al.(2021)]{brattka2021}
V.~Brattka, G.~Gherardi, and A.~Pauly.
\newblock Weihrauch complexity in computable analysis.
\newblock In V.~Brattka and P.~Hertling, editors,
  \emph{Handbook of Computability and Complexity in Analysis},
  pages 367--417. Springer, 2021.
...
\bibitem[Lee(2026)]{Lee26format}
P.~C.-K.~Lee.
\newblock Paper format guide for the Constructive Reverse Mathematics Series.
\newblock Preprint, 2026.
\newblock DOI: \href{https://doi.org/10.5281/zenodo.18765700}{10.5281/zenodo.18765700}.
