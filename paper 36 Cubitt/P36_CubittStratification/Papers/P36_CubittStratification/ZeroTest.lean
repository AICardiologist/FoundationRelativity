/-
  ...
  CRM nuance: The positive formulation Δ = 0 ∨ Δ > 0 is LPO, not WLPO.
  The WLPO formulation is Δ = 0 ∨ Δ ≠ 0 (negative second disjunct).
  Passing from Δ ≠ 0 to Δ > 0 (for Δ ≥ 0) requires Markov's Principle,
  and WLPO + MP ⟺ LPO.
-/
...
-- This is equivalent to WLPO.
-- NOTE: Δ = 0 ∨ Δ > 0 would be LPO (positive second disjunct).
-- WLPO uses the *negative* disjunct Δ ≠ 0.
...
/-- THEOREM 4 (WLPO): The physical gap zero-test Δ = 0 ∨ Δ ≠ 0
    ...
    CRM nuance: The positive formulation Δ = 0 ∨ Δ > 0 is LPO.
    WLPO uses the negative disjunct: Δ ≠ 0 means ¬(Δ = 0), which
    for a non-negative real does not constructively imply Δ > 0.
    That upgrade requires Markov's Principle, and WLPO + MP = LPO.
    ...
    Connection to Paper 33: The QCD mass gap asks whether Δ_QCD ≠ 0,
    which is a WLPO zero-test on a specific Hamiltonian's gap. -/
