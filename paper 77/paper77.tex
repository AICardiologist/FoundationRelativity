...
% ---------- Title and author ----------
\title{The Complete Constructive Hodge Theorem\\
  for CM Abelian Fourfolds:\\
  Automated De-Omniscientisation via DAG Surgery\\[6pt]
...
\begin{abstract}
The Hodge Conjecture---that every rational $(p,p)$-class on a
smooth projective variety is algebraic---has been verified for
abelian varieties with complex multiplication by
Deligne~(1982), Lieberman~(1968), and others, but always through
classical existence arguments that produce no explicit algebraic
representatives.  For CM abelian fourfolds such as~$E^4$
($E$ an elliptic curve with $\End(E) \otimes \Q \cong \Q(i)$),
the Hodge~$(2,2)$ space is 36-dimensional; classical proofs
assert that divisor products span this space without exhibiting a
single decomposition.

We prove the \emph{Complete Constructive Hodge Theorem for~$E^4$}:
every Hodge~$(2,2)$ class is an explicit $\Q$-linear combination
of cup products of divisor classes, with all 36 basis
decompositions formally verified in Lean~4 by
\texttt{native\_decide} (Theorem~B).  As a prerequisite, we prove
there are \emph{no exotic Weil classes} on~$E^4$:
$\dim_\Q \Hdg^{2,2} = \dim_\Q(\text{div.\ products}) = 36$
(Theorem~A), refuting the initial hypothesis that the Anderson
obstruction applies to this variety.  The Lean bundle builds with
0~sorry and 0~\texttt{noncomputable}; the classical Hodge
existence axiom is present but provably unused.

These results are produced by the \emph{CRMLint Squeeze}, a
protocol for reverse-engineering classical proofs into constructive
ones (Theorem~C).  The method identifies the \emph{Classical
Boundary Node} (CBN)---the exact lemma where a classical proof
invokes an omniscience principle---excises it, and replaces it
with an explicit algebraic witness.  The paper introduces the
\emph{asymmetric offloading} architecture: Python CAS computes
exact $\Q$-linear algebra, emits Lean~4 source with hardcoded
rational data, and the Lean kernel verifies via
\texttt{native\_decide}.

This is the first time the Hodge Conjecture for a non-trivial
abelian variety has been machine-verified as a finite constructive
database with explicit rational witnesses, replacing the classical
existence theorem with 36 computable decompositions.
\end{abstract}
...
We provide the first \emph{explicit constructive proof}: 36
rational decompositions, each expressing a Hodge basis vector as a
$\Q$-linear combination of cup products of divisor classes,
formally verified in Lean~4.  This replaces the classical
existence theorem (Lieberman/Deligne) with a finite computable
database.  The proof also establishes that $E^4$ supports no
exotic Weil classes, resolving the question of whether the
Anderson obstruction applies to non-simple products.
...
The CRMLint Squeeze transitions the CRM program---which
traces its intellectual lineage to Kronecker's~\cite{Kronecker1882}
insistence on explicit construction---from auditing
