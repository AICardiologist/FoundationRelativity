...
\title{%
  ...
  {\normalsize A Lean~4 Formalization (Paper~38, Constructive Reverse Mathematics Series)}%
}
...
The entire formalization (577~lines of \Lean{}/\Mathlib{})
...
theorem wang_tiling_iff_lpo :
    (∀ (W : WangTileset),
      tiles_plane W ∨ ¬tiles_plane W) ↔ LPO :=
...
The Lean formalization encodes all seven entries as a list of
\texttt{GenealogyEntry} records and verifies the classification
by \texttt{decide} (kernel computation).
\end{proof}

\begin{remark}[Watson--Cubitt exclusion]\label{rem:wc-exclusion}
Watson and Cubitt~(2022) proved that the \emph{ground state energy density}
problem is computationally hard (QMA-complete for finite lattices).
This is \emph{not} an undecidability result: QMA-hardness is a
complexity-theoretic statement about finite systems, not a halting
reduction.  The ground state energy density for a fixed Hamiltonian
at finite volume is a finite eigenvalue problem, computable in
$\BISH$.  Paper~37's meta-theorem correctly identifies this as
$\BISH$-computable.  Earlier versions of this paper incorrectly
included Watson--Cubitt in the genealogy; this has been corrected.
\end{remark}
...
\caption{The genealogy of physical undecidability.
  7~entries, all \LPO-equivalent (verified by \texttt{decide}).
  Watson--Cubitt (2021) ground state energy density is excluded:
  it is a $\BISH$-computable finite eigenvalue problem (Paper~37),
  not an undecidability result.}
\label{tab:genealogy}
\end{table}
...
\draw (ceil) -- (main);
\end{tikzpicture}
\caption{Module dependency graph (577~lines, 7~modules).}
\label{fig:arch}
\end{figure}
...
\bibitem{Lee26P29}
P.~C.-K.~Lee,
``Fekete's subadditive lemma and LPO,''
Zenodo, 2026. Paper~29.
DOI: \href{https://doi.org/10.5281/zenodo.18643617}{10.5281/zenodo.18643617}.

\bibitem{Lee26P36}
P.~C.-K.~Lee,
``Stratifying spectral gap undecidability: Cubitt's theorem is LPO,''
Zenodo, 2026. Paper~36.
DOI: \href{https://doi.org/10.5281/zenodo.18642620}{10.5281/zenodo.18642620}.

\bibitem{Lee26P37}
P.~C.-K.~Lee,
``The halting reduction meta-theorem: every halting reduction is LPO,''
Zenodo, 2026. Paper~37.

\bibitem{Lee26FormatGuide}
P.~C.-K.~Lee,
