...
/-- The *bidual gap* pseudo‑functor.  Currently only the object/1‑cell maps
    are populated; coherence proofs are delegated to `Math‑AI`. -/
def GapFunctor : PseudoFunctor where
  obj      := fun F => F        -- placeholder: refine as needed
  map₁     := fun {A B} f => f  -- will change once dualisation inserted
