/-
  ...
  Defs/EncodedAsymmetry.lean — Encoded KS asymmetry construction
  ...
  and define the "KS asymmetry" as their difference.

  The sign of the asymmetry (≤ 0 vs ≥ 0) determines which parity
  class contains the unique true value (if any).

  This is structurally identical to Paper 21's Bell asymmetry —
  the encoding is domain-independent. The physical interpretation
  differs: here the asymmetry tracks which class of measurement
  contexts lacks predetermined values.
-/
...
/-- The KS asymmetry: difference between even-field and odd-field signals.
    Physically, this represents the imbalance between "even-indexed" and
    "odd-indexed" measurement context failures, parameterized by α.

    Structurally identical to Paper 21's Bell asymmetry — the encoding
    is domain-independent; only the physical interpretation differs. -/
def ksAsymmetry (α : ℕ → Bool) : ℝ :=
  evenField α - oddField α

/-- The KS sign decision: for every α with AtMostOne, the asymmetry
    has a decidable sign. This is the physical formulation of LLPO
    in the Kochen-Specker contextuality context. -/
