...
\author{Paul C.\ K.\ Lee\\
\small Department of Medicine, New York University Grossman School of Medicine\\
...
\begin{tikzpicture}[
  ...
  % Edges
  \draw[->, thick] (num) -- node[above, font=\footnotesize]
    {sacrifice faithfulness} (hom);
  \draw[->, thick] (hom) -- node[above, font=\footnotesize, sloped]
    {sacrifice $\BISH$} (num);
  ...
  % Labels
  \node[below=0.15cm of num, font=\scriptsize, text=blue!60!black]
    {Jannsen escape};
  \node[below=0.15cm of hom, font=\scriptsize, text=red!60!black]
    {costs $\LPO$};
...
'axiom1_characterisation' depends on axioms:
  [conjD_morphism_cost, conjD_morphism_cost_eq,
   no_conjD_morphism_cost, no_conjD_morphism_cost_eq]
...
or \texttt{Quot.sound}.  The only axioms are the four custom declarations
in \cref{tab:axioms}, all with mathematical references.
