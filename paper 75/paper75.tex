...
\end{equation}
Papers~72--74 established the DPT axiom trilogy as a system of
biconditionals: each of the three DPT axioms for motivic arithmetic
is the unique condition ensuring its associated operation is
$\BISH$-decidable.  The axiom system is canonical, not merely
minimal.
...
\begin{itemize}
  ...
  \item $\CLASS$ (full classical logic): the Law of Excluded
    Middle for all propositions.  Equivalent to $\LPO$ for all
...
findings into the programme's final synthesis.
...
The solidification functor $\mathrm{Solid} \colon \mathrm{CondAb}
\to \mathrm{SolidAb}$ projects a condensed abelian group onto its
solid part.  For light condensed groups, solidification computes as
an inverse limit over the category of finite sets, where the
transition maps are surjections of finite sets.
...
center of the category of smooth representations of~$G(F)$.
...
\begin{proof}
Solidification of a light condensed abelian group~$M$ computes as
$M^{\mathrm{solid}} = \varprojlim_{S \in \mathrm{FinSet}^{\mathrm{op}}}
M(S)$, an inverse limit indexed by the category of finite sets.

The key observation: every surjection of finite sets splits.  Given
$f \colon S \twoheadrightarrow T$ with $S, T$ finite, a section
$s \colon T \to S$ exists by choosing one preimage per element ---
a finite process requiring no axiom of choice.

Consequence: the transition maps in the inverse system are split
epimorphisms.  The Mittag-Leffler condition holds trivially (the
image stabilizes immediately), so $\varprojlim^1 = 0$
constructively.  The inverse limit exists and is computable in~$\BISH$.

This corrects earlier programme estimates (see
\S\ref{sec:correction}) that placed solidification at $\LPO$
via Mittag-Leffler and Dependent Choice.  In fact, the finiteness of
the index sets makes $\DC$ unnecessary.
\end{proof}
...
The residual operation is verifying
$\mathrm{trace}(\pi)(f) = \mathrm{trace}(\varphi)(f)$ for test
functions~$f$.  This is an equality test on values that may be
continuous/algebraic parameters.  By Paper~74 (Theorem~C),
eigenvalue/trace comparison costs $\WLPO$ when the parameters are
not guaranteed to have geometric origin.  The Bernstein center
extraction gives~$\varphi$ algebraically, but the trace values
themselves involve spectral data whose exact equality requires
$\WLPO$.
...
$\CRM(\text{GL statement}) = \WLPO < \CLASS = \CRM(\text{FS proof})$.
...
Paper~68 established the same pattern for Fermat's Last Theorem:
...
Earlier programme estimates (see~\cite{roadmap}, Open Question~4)
...
% Statement (separate)
\node[layer, fill=yellow!20, minimum width=4cm] (stmt) at (7, 0)
  {GL LLC Statement\\$\WLPO$};

% Gap indicator
\draw[arrow, dashed, red!60!black] (stmt.north) --
  node[right, label] {conservation gap}
  ++(0, 3.5) node[above, font=\small\bfseries] {$\CLASS$};

% Annotations
\node[label, anchor=west] at (3.2, 0) {$\BISH \leq \WLPO$};
\draw[arrow, dotted, gray] (alg.east) -- (stmt.west);
...
Paper~67 (the programme's synthesis monograph) will incorporate
...
\end{enumerate}
Together, these support the programme's central thesis: the logical
...
\bottomrule
\end{tabular}
\caption{De-omniscientizing descent across the programme.  The
function field LLC (Paper~69) has zero gap: the proof is already
$\BISH$.  FLT (Paper~68) and the GL LLC (Paper~75) exhibit
conservation gaps.}
\label{tab:descent-comparison}
\end{table}

The function field case (Paper~69) is instructive: L.~Lafforgue's
proof of the function field LLC is already $\BISH$, with zero
conservation gap.  The difference is that function fields have
algebraic spectral parameters (Remark~3.3 of Paper~69: $z = q^{-s}$
lies on a compact algebraic torus), so the trace formula's
eigenvalue comparison is exact algebraic arithmetic.  Over number
fields, the analytic Langlands spectrum introduces continuous
parameters, forcing the statement cost up to $\WLPO$.
...
previous programme results.  Paper~69 showed the function field LLC
...
The programme's central thesis --- the logical cost of mathematics
...
The author thanks the AI research assistants (Claude, Anthropic;
Gemini, Google) for their contributions to this work.  The three-layer
stratification was developed in consultation with Gemini.  The formal
verification was developed and checked with Claude (Anthropic).  The
Lean~4 code uses the Mathlib library~\cite{mathlib}.  The logical
structure of the main results has been verified by formal proof; the
