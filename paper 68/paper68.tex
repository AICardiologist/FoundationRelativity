...
% ---------- Lean repo link ----------
\newcommand{\leanRepo}{\url{https://doi.org/10.5281/zenodo.18901011}}
...
audit of Wiles's proof that the $p$-adic Galois representation
$\rho_{E,p}$ attached to every semistable elliptic curve
$E/\Q$ is modular (\emph{Galois modularity}:
$\rho_{E,p} \cong \rho_{f,p}$ for some eigenform~$f$),
and hence of Fermat's Last Theorem.
\emph{Geometric modularity} (that $E$ is isogenous to the
abelian variety $A_f$) additionally requires Faltings's Isogeny
Theorem, whose Archimedean content places it at~$\CLASS$; FLT
does not need it, since Ribet's level-lowering operates entirely
on~$\bar{\rho}_{E,p}$.
The proof decomposes into five stages: residual modularity
...
Our principal finding is an \emph{asymmetry}: Stages~2--4 are
fully constructive ($\BISH$), while Stage~1 requires $\WLPO$
(Weak Limited Principle of Omniscience) and Stage~5
(Taylor--Wiles patching) requires $\WKL$
(the inverse limit of finite local rings).
The non-constructive content of Wiles's proof is concentrated
at two points: the analytic theory of weight~1 automorphic forms
(Stage~1) and the patching compactness argument (Stage~5).
...
($\GL_2(\F_2) \cong S_3$), where Hecke theta series
(requiring only classical Poisson summation, not the trace formula)
provide a $\BISH$ construction.
The overall classification of Fermat's Last Theorem is therefore
$\WKL$: the $\WLPO$ was an artefact of Wiles's
choice of residual prime, but the $\WKL$ in Taylor--Wiles
patching (inverse limit of finite local rings) is irreducible.

Two post-Wiles developments partially reduce the Stage~5
classification: Brochard's proof of de~Smit's conjecture (2017),
which reduces the freeness verification to finite level, and
unconditional effective Chebotarev bounds
(Lagarias--Montgomery--Odlyzko 1979, Ahn--Kwon 2019), which make
the Taylor--Wiles prime search a bounded computation.
The residual $\WKL$ is irreducible: Taylor--Wiles patching
constructs $R_\infty$ as an inverse limit of finite local rings,
which is equivalent to finding an infinite path in a finitely
branching tree (Paper~98, Calibration Theorem~II).
...
Throughout, ``modularity'' means \emph{Galois modularity}: the
$p$-adic representation $\rho_{E,p}$ is isomorphic to the
representation attached to a weight~2 eigenform.  Ribet's
level-lowering theorem operates on $\bar{\rho}_{E,p}$ and yields the
FLT contradiction $S_2(\Gamma_0(2)) = 0$ without ever requiring
that $E$ be isogenous to~$A_f$ (geometric modularity, which
requires Faltings's Isogeny Theorem: $\CLASS$).
We establish five results:

\begin{description}[leftmargin=2em]
\item[Theorem A] (Stage~5 is $\WKL$).
The Taylor--Wiles prime search is $\BISH$: effective Chebotarev
bounds \cite{LMO1979, AhnKwon2019} eliminate Markov's Principle,
and Brochard's finite-level criterion \cite{Brochard2017} reduces
the freeness verification to level~$n = 2$.  However, the
patching construction itself---forming $R_\infty$ as an
inverse limit of finite local rings---requires $\WKL$
(Paper~98, Calibration Theorem~II).  The logical cost descends
from $\MP + \FT$ (1995) to $\WKL$ (2017/2019).
...
The overall classification of Wiles's proof is $\WLPO$
(join of $\WLPO$ from Stage~1 and $\WKL$ from Stage~5).
...
(projective image~$S_3 = D_3$, requiring only Hecke theta series
and Poisson summation).  The resulting proof establishes Galois
modularity at~$\WKL$: the sole non-constructive node is
...
\subsection{Theorem A: Stage~5 is \texorpdfstring{$\WKL$}{WKL}}\label{sec:stage5}
...
Brochard's criterion reduces the freeness verification to a
finite-level algebraic check, eliminating the full Fan Theorem.
The residual $\WKL$ is the inverse limit that constructs
$R_\infty$ from compatible finite approximations.
...
Stage~5 of Wiles's proof is~$\WKL$.  The prime search ($\MP$)
is eliminated by Proposition~\ref{prop:tw}; Brochard's criterion
(Theorem~\ref{thm:brochard}) reduces the freeness verification to
finite level.  However, the patching construction
itself---forming $R_\infty$ as an inverse limit of finite
local $\Z_p$-algebras---requires $\WKL$: it is equivalent to
finding an infinite path in a finitely branching tree.
\end{theorem}

\begin{proof}
Given axioms B1 (Brochard's criterion), B2 (effective Chebotarev),
and B3 (discriminant computability), the \emph{search} component of
Stage~5 proceeds as:
...
Steps (1)--(4) are finite, decidable computations ($\BISH$).
The residual $\WKL$ is in the patching \emph{assembly}: the
inverse limit that constructs $R_\infty$ from compatible finite
approximations (Paper~98, Calibration Theorem~II).
\end{proof}
...
Brochard + effective Chebotarev & 2017/2019 & $\WKL$ \\
\bottomrule
\end{tabular}
\end{center}
...
foundations.  The community reduced the logical cost of the
proof of FLT from $\MP + \FT$ to $\WKL$ without knowing it.
...
\midrule
Theorem A (Stage~5 is $\WKL$) & $\WKL$ & Yes (from axioms) & Yes \\
...
Theorem D (Asymmetry) & $\WLPO$ & Yes (join) & Yes \\
\bottomrule
\end{tabular}
\end{center}
...
\[
\underbrace{\MP + \FT}_{\text{1995: TW original}} \;\;\xrightarrow{\quad\text{Brochard + eff.\ Chebotarev}\quad}\;\; \underbrace{\WKL}_{\text{2017: inverse limit only}}.
\]
The mechanism: effective Chebotarev bounds the prime search
(eliminating $\MP$), and Brochard's finite-level criterion
reduces the freeness verification.  The residual $\WKL$
is the inverse limit itself: the patching construction
$R_\infty = \varprojlim R_{Q_n}$ requires an infinite path
in a finitely branching tree.
...
\texttt{Paper68\_Stage5.lean} & 178 &
  Target~1: Stage~5 prime search is $\BISH$; patching is $\WKL$ \\
...
are~$\BISH$ and specialisation is~$\BISH$.  However, the
Kassaei--Pilloni classicality theorem relies on rigid analytic
geometry (Tate), whose foundation requires locating maximal
ideals of Tate algebras---structurally invoking Krull's Maximal
Ideal Theorem (equivalent to the Boolean Prime Ideal theorem,
hence~$\CLASS$).  The constructive status of classicality
...
% ============================================================
\section{The dihedral bypass: \texorpdfstring{$\CRM(\mathrm{FLT}) = \WKL$}{CRM(FLT) = WKL}}%
\label{sec:bypass}
% ============================================================
...
Hecke's construction avoids the trace formula entirely---it
uses lattice sums and the algebraic theory of quadratic forms,
with no $L^2$ spectral decomposition and no continuous spectrum.
The functional equation for the theta series requires
Poisson summation over~$\R$ (Archimedean analysis), but
Poisson summation is constructively valid
(Bishop \cite{Bishop1967}).  The construction is~$\BISH$.
...
classified as~$\WKL$ in \S\ref{sec:stage5}, together with
...
\begin{theorem}[Kisin's $p = 2$ modularity lifting is $\WKL$]%
\label{thm:kisin2}
The modularity lifting theorem for $2$-adic Barsotti--Tate
representations (Kisin \cite{Kisin2009}) is~$\WKL$.
The proof uses finite flat group scheme classification
(commutative algebra: $\BISH$), Taylor--Wiles patching ($\WKL$,
Theorem~A), and effective Chebotarev ($\BISH$).
No analytic input from the trace formula is required.  The sole
non-constructive node is the patching inverse limit.
\end{theorem}

\subsection{Completing the proof}

\emph{Decidability of the case-split.}  For an elliptic curve
$E/\Q\colon y^2 = x^3 + ax + b$, the representation
$\bar{\rho}_{E,2}$ is absolutely irreducible if and only if the
$2$-torsion field $\Q(E[2])$---the splitting field of
$f(x) = x^3 + ax + b$---has degree~$6$ over~$\Q$, equivalently
$f$ is irreducible over~$\Q$.  Because polynomial factorisation
over~$\Q$ is unconditionally $\BISH$-decidable (Kronecker's
algorithm: test divisibility by all integer polynomials of
bounded degree and coefficient size), the branch between the two
cases below is fully constructive.
...
\item $\rho_{E,2} \cong \rho_{f,2}$ for a weight~2
  eigenform~$f$ (Galois modularity, $\BISH$).  Ribet's
  level-lowering applies to $\bar{\rho}_{E,p}$ directly,
  bypassing Faltings's Isogeny Theorem.
\end{enumerate}
...
modularity lifting theorem (Theorem~A, $\WKL$) yields
...
\begin{theorem}[$\CRM(\mathrm{FLT}) = \WKL$]%
\label{thm:flt-wkl}
The proof of Fermat's Last Theorem via the $p = 2$ dihedral
bypass (Kisin \cite{Kisin2009}, Khare--Wintenberger
\cite{KhareWintenberger2009I}) replaces Stage~1
(Langlands--Tunnell, $\WLPO$) with the dihedral base case
(Hecke theta series, $\BISH$).  Stages~2--4 are $\BISH$ by
Theorem~B.  Stage~5 (Taylor--Wiles patching) is $\WKL$ by
Theorem~A.  Therefore:
\[
  \CRM(\mathrm{FLT}) \;=\; \WKL.
\]
The $\WKL$ is irreducible: Taylor--Wiles patching is equivalent
to $\WKL$ (Paper~98, Calibration Theorem~II).
\end{theorem}
...
Lifting & Taylor--Wiles ($\WKL$) & Kisin ($\WKL$) \\
\midrule
\textbf{Total} & $\WLPO$ & $\WKL$ \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{Higher-dimensional modularity lifting.}
  ...
  non-self-dual representations.  If patching remains $\WKL$ in
...
First, Wiles's 1995 proof is $\WLPO$ (join of $\WLPO$ from
Stage~1 and $\WKL$ from Stage~5).  The $\WLPO$ is localised
in Stage~1 (Langlands--Tunnell); the $\WKL$ is localised in
Stage~5 (Taylor--Wiles patching).
...
The $\WKL$ in Taylor--Wiles patching is irreducible.

\[
  \boxed{\CRM(\mathrm{FLT}) \;=\; \WKL.}
\]

The sole non-constructive node in the proof of Fermat's Last
Theorem is the inverse limit of finite local rings in
Taylor--Wiles patching---a non-Archimedean compactness argument
equivalent to $\WKL$ (Paper~98, Calibration Theorem~II).
The $\WLPO$ in Wiles's original argument was the cost of a
specific proof \emph{strategy} (the choice of $p = 3$), not a
property of the \emph{theorem}.  Geometric modularity
($E \sim A_f$, requiring Faltings) remains~$\CLASS$, but FLT
needs only Galois modularity ($\rho_{E,p} \cong \rho_{f,p}$).
Whether $\CRM(\mathrm{FLT}) = \BISH$---eliminating the $\WKL$
patching node---remains open; it would require a modularity
lifting theorem without inverse limits.
...
mathematical arguments supporting the stage-cost assignments have been
