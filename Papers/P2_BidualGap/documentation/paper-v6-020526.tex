...
Assuming $\WLPO$ and a witness-extraction step equivalent to LPO for
near-supremum coefficients in $\ell^1$, the canonical embedding
$J:c_0\to(c_0)^{**}$ is not surjective.
\end{thm}
...
exactly the LPO-style witness-extraction hypothesis stated above.
...
this step in Lean and supply an LPO-based variant that \emph{does} provide such an
index. Concretely, the lemma
\texttt{exists\_coeff\_near\_sup\_LPO} yields
\texttt{dual\_is\_banach\_l1\_from\_LPO} and
\texttt{dual\_is\_banach\_c0\_dual\_from\_LPO} in
\texttt{Papers.P2\_BidualGap.HB.WLPO\_DualBanach}, which we use to bound $G$ above.
The earlier WLPO-only phrasing should therefore be read as conditional on this
explicit witness-extraction axiom (LPO or a Markov-style principle).
