...
This equivalence builds on and refines earlier connections between variants of Hahn--Banach and omniscience principles in constructive reverse mathematics (e.g.\ Ishihara; Bridges--Richman; Diener) by isolating the \emph{bare bidual-gap statement} and showing it sits precisely at \WLPO. Our Lean~4 development separates the (meta-classical) witness-extraction step from the (constructive) analysis that derives \WLPO from an abstract kernel, making the logical hygiene explicit. We also include a constructively careful account of the Boolean algebra of idempotents in $\ell^\infty/c_0$ (``Stone window'') and finite-dimensional surrogates via Cesàro means. 
...
\item \textbf{Non-reflexivity folklore.} That $\ell^\infty$ is non-reflexive is a textbook fact (e.g.\ \cite{AlbiacKalton16}). The \emph{logical} content of the \emph{bare non-reflexivity statement} over \BISH\ appears not to have been recorded explicitly; our contribution is to formulate and verify the bi-implication with \WLPO.
\end{itemize}
...
\smallskip
\noindent\textbf{AI/Tooling disclosure.} The Lean code and parts of the prose benefited from standard programming assistants (e.g.\ GPT-5 Pro, Gemini, Claude) under human supervision. All mathematical claims were reviewed by the author and are machine-checked by Lean~4. Any errors are the author's responsibility.
