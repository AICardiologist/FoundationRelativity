...
\item \lean{concat\_left\_nest\_eq}: Left--nested reassociation
\item \lean{Extendω\_is\_lub}: $\omega$ as least upper bound
\item \lean{ExtendIter\_concat\_left\_nest\_eq}: Stage--level corollary
\end{enumerate}
...
\item \textbf{Order Theory}: Lightweight $\leq^T$ layer emerged naturally
