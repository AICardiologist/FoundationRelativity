...
A second part uses the classical Stone window isomorphism $\mathrm{Idem}(\linf/\cnull)\cong\mathcal{P}(\N)/\mathrm{Fin}$ as a calibration case study, measuring which constructive principles are required to recover surjectivity for various support ideals. Finally we discuss Lean~4 considerations: current bicategory infrastructure and practical workarounds for a near--term mechanization (strict $2$--category skeletons, thin $2$--cells, and displayed encodings).
\end{abstract}
...
\begin{mdframed}[style=status]
\textbf{Current Status:} The Lean 4 formalization is mathematically complete with 0 sorries in all proven components. Part 6 (exact finish time characterization) is partially complete with the upper bound proven and lower bound remaining.
\end{mdframed}
...
Part V: Collision Theorems & $\checkmark$ Structure & Axiomatized \\
Part VI: Stone Window & $\checkmark$ Complete & 0 sorries \\
\hline
...
Part 6B: Lower Bound & \textbf{TODO} & Constructive proof needed \\
Part 6C: Exactness & \textbf{TODO} & Combine upper+lower \\
Part 6D: General Case & \textbf{TODO} & Permutation with Finset \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
\item Complete 2--categorical foundation--relativity theory with 0 mathematical sorries
\item Uniformization height = 1 for bidual gap (fully proven)
\item k--ary schedule abstractions with complete quota invariants
\item Upper bound proof: $N^* = k(H-1) + S$ suffices for packed axes
\item Complete proof that k=2 schedule exactly matches \texttt{fuseSteps} pattern
\end{itemize}

\subsection{Remaining Work for Part 6}
To complete the exact finish time characterization:
\begin{enumerate}
\item \textbf{Lower Bound (Part 6B):} Prove that $\forall n < N^*$, $\exists i$ with $h(i) > \text{quota}(i, n)$
   \begin{itemize}
   \item Strategy: Constructive case analysis on $n = k \cdot m + r$
   \item If $m \leq H-2$: all quotas $\leq H-1$, pick any $i < S$
   \item If $m = H-1$ and $r < S$: pick $i = r$, its quota is $H-1$
   \end{itemize}
\item \textbf{Exactness (Part 6C):} Establish that targets are met $\Leftrightarrow n \geq N^*$
\item \textbf{General Case (Part 6D):} Generalize from packed to arbitrary axis orderings via permutation
\item \textbf{Integration:} Wire into ProductHeight theorems for k--ary products under AxisIndependent
\end{enumerate}
...
\textbf{k--ary Schedule Infrastructure (Parts 1--5): \checkmark COMPLETE}
\begin{itemize}
\item[\checkmark] \texttt{Schedule} structure for k--ary proof scheduling with quota invariants
\item[\checkmark] Round--robin scheduling with \texttt{roundRobin\_assign} lemma
\item[\checkmark] Complete proof that k=2 schedule exactly matches \texttt{fuseSteps} pattern
\item[\checkmark] Block/bridge lemmas: \texttt{roundRobin\_block\_start\_bridge}, \texttt{roundRobin\_k1\_bridge}
\item[\checkmark] Migration path: \texttt{evenOdd\_eq\_fuseSteps\_even/odd}
\end{itemize}

\textbf{Part 6 -- Exact Finish Time Mathematics:}
\begin{itemize}
\item[\checkmark] \textbf{Part 6A (Upper Bound)}: COMPLETE
  \begin{itemize}
  \item \texttt{quota\_roundRobin\_block\_closed}: Quota at $k \cdot n + r = n + \mathbf{1}_{i < r}$
  \item \texttt{quotas\_reach\_targets\_iff}: Feasibility $\Leftrightarrow q_i \leq \lfloor n/k \rfloor + \mathbf{1}_{i < n \bmod k}$
  \item \texttt{quotas\_reach\_targets\_packed}: Upper bound at $N^* = k(H-1) + S$ (packed setting)
  \end{itemize}
\item[\textbf{TODO}] \textbf{Part 6B (Lower Bound)}: IN PROGRESS
  \begin{itemize}
  \item \texttt{quotas\_not\_reached\_below\_packed}: Prove $\forall n < N^*$, $\exists i$ with $h(i) > \text{quota}(i, n)$
  \item Strategy: Constructive case analysis on $n = k \cdot m + r$
  \end{itemize}
\item[\textbf{TODO}] \textbf{Part 6C (Exactness)}: NOT STARTED
  \begin{itemize}
  \item Define $N^*(k, H, S) := \begin{cases} 0 & H = 0 \\ k(H-1) + S & H > 0 \end{cases}$
  \item Prove: Targets met $\Leftrightarrow n \geq N^*$ (combining 6A and 6B)
  \end{itemize}
\item[\textbf{TODO}] \textbf{Part 6D (General Case)}: NOT STARTED
  \begin{itemize}
  \item Permutation lemma using Finset (10--20 lines)
  \item Integration with \texttt{ProductHeight} theorems under \texttt{AxisIndependent}
  \end{itemize}
\end{itemize}
\end{itemize}
...
\paragraph{D3 (Profiles):} Orthogonal profiles and frontiers updated to include UCT and Baire calibrators; transfer applications via Prop.~\raef{VI:prop:transfer}.
