...
\date{August 2025 \\[2pt] \small (working note / preprint)}  %% NEW/CHANGED
...
%% NEW/CHANGED: Motivation subsection
...
\begin{remark}[Mechanization status (working note)]  %% NEW/CHANGED
...
\subsection*{Key signatures (excerpt)}  %% NEW/CHANGED (fixed symbols)
\begin{lstlisting}
...
\paragraph{Toolchain.} Lean toolchain: \texttt{leanprover/lean4 v4.23.0-rc2} (as pinned via \texttt{.elan}); \texttt{mathlib4} pinned to commit \texttt{32a7e535287f9c7340c0f91d05c4c20631935a27}.  %% NEW/CHANGED
...
\paragraph{Spectrum stub (temporary).}  %% NEW/CHANGED
...
\section{Limitations and roadmap}  %% NEW/CHANGED
\begin{itemize}
