...
lemma dCoord_sub_sub_regroup
  ...
  have h₂ :
    ...
    simpa using
      dCoord_add_of_diff μ Y Z r θ hYr hZr hYθ hZθ
  -- Assemble.
  calc
    dCoord μ (fun r θ => X r θ - Y r θ - Z r θ) r θ
...
@[simp] lemma sumIdx_add3 (f g h : Idx → ℝ) :
  ...
  classical
  -- Expand Σ(f+g+h) as Σ((f+g)+h) and distribute once per step.
  have := sumIdx_add_distrib (fun i => f i + g i) h
  have := by
    simpa using this
  -- Break Σ(f+g) into Σf + Σg
  have hfg : sumIdx (fun i => f i + g i) = sumIdx f + sumIdx g := by
    simp
  -- Assemble: (Σ(f+g)) + Σh = (Σf + Σg) + Σh
  simp [add_assoc]
...
lemma sumIdx_add3_of_sub (f g h k : Idx → ℝ) :
  ...
  have h' := (sumIdx_add3_rev f g (fun i => h i - k i))
  simp
...
@[simp] lemma scalar_pack4_distrib (A B C D g : ℝ) :
  ...
  -- single, local normalization; keep distributivity out of the big goal
...
=
  ...
  have h₃ :
    ...
    simpa using (sumIdx_add_same_left G (fun ρ => A ρ - B ρ) C)
...
lemma dCoord_g_differentiable_r_ext
  ...
  | φ =>
      simp [DifferentiableAt_r, dCoord]
  | θ =>
      simpa using diff_r_dCoord_θ_g M r θ a b
  | r =>
      cases a <;> cases b
      case t.t =>
...
=
  ...
  have ΓΓ_cross_collapse_b_μ :
    ...
    have hdiag :
      ...
      simpa [mul_comm, mul_left_comm, mul_assoc]
        using sumIdx_reduce_by_diagonality M r θ ρ (fun e => Γtot M r θ e ν b)
    ring_nf
  ...
  have ΓΓ_cross_collapse_b_ν :
    ...
    have hdiag :
      ...
      simpa [mul_comm, mul_left_comm, mul_assoc]
        using sumIdx_reduce_by_diagonality M r θ ρ (fun e => Γtot M r θ e μ b)
    ring_nf
    simp [mul_comm, mul_left_comm, mul_assoc]
  ...
  have ΓΓ_cross_collapse_a_μ :
    ...
    have hdiag :
      ...
      simpa [mul_comm, mul_left_comm, mul_assoc]
        using sumIdx_reduce_by_diagonality M r θ ρ (fun e => Γtot M r θ e ν a)
    ring_nf
    simp [mul_comm, mul_left_comm, mul_assoc]
  ...
  have ΓΓ_cross_collapse_a_ν :
    ...
    have hdiag :
      ...
      simpa [mul_comm, mul_left_comm, mul_assoc]
        using sumIdx_reduce_by_diagonality M r θ ρ (fun e => Γtot M r θ e μ a)
    calc
      (Γtot M r θ ρ ν b) * sumIdx (fun e => Γtot M r θ e μ a * g M ρ e r θ)
...
lemma RiemannUp_r_φrφ_ext
  ...
  have hderφφ : deriv (fun s => Γ_r_φφ M s θ) r = -(Real.sin θ)^2 := by
    ...
    have : deriv (fun s : ℝ => -(s - 2 * M) * (Real.sin θ)^2) r = -(Real.sin θ)^2 := by
      simpa [deriv_mul_const, deriv_neg, h1]
