...
\medskip
\noindent\textbf{Artifact status (as of this draft):} no \texttt{sorry} placeholders in the GR development; GR target build is $\sim$2m10s (warm cache) and a clean full-repo build is $\sim$1h15m on the author machine (details in Section~\ref{sec:artifact}).
\end{abstract}
...
\textbf{Clean full-repo build time:} 1h 14m 53s (cold cache) \\
\textbf{GR target build time:} 2m 10s for \texttt{Papers.P5\_GeneralRelativity.GR.Riemann} (warm cache)
\end{mdframed}
...
| Idx.θ, Idx.θ, r, _ => r^2
| Idx.φ, Idx.φ, r, theta => r^2 * (Real.sin theta)^2
| _, _, _, _ => 0
\end{leancode}

\paragraph{Christoffel aggregator.}
\begin{leancode}
noncomputable def Γtot (M : ℝ) : Idx → Idx → Idx → ℝ → ℝ → ℝ
...
\begin{leancode}
rw [sumIdx2_expand]   -- expand ∑ μ ∑ ν into 16 concrete terms
\end{leancode}
...
simp only [Riemann_last_equal_zero,
  R_tr_tθ_zero, R_tr_tφ_zero,  -- (many more)
  ...]
\end{leancode}
...
\begin{itemize}
\item \texttt{Schwarzschild.lean}: defines the Schwarzschild metric $g$, its inverse $g^{-1}$, and the nonzero Christoffel symbols $\Gamma^a_{bc}$ (collected in \texttt{Γtot}). It proves the required derivative lemmas for $f(r)=1-\frac{2M}{r}$ and the basic smoothness facts on the exterior domain.
...
\item \texttt{Γtot\_symm}, \texttt{g\_symm}: symmetry lemmas for Christoffel and metric components.
...
\item \texttt{simp\_rw} for controlled rewriting of sumIdx/Γtot expansions
...
Certificates.lean-+        |                    |                  |
  (AxCal stubs)            |                    |                  |
                           v                    v                  v
                    Γtot, g, f-derivs  Riemann_via_Gamma1, R_*_zero  sixBlock, Kretschmann_value
\end{Verbatim}
...
\item \textbf{Riemann tensor.} Define $R^\rho{}_{\sigma\mu\nu}$ by the coordinate formula above. Expand each nonzero component with the \texttt{Γtot} aggregator, use symmetry identities
...
\item Index type design (\texttt{Idx}, \texttt{sumIdx}, \texttt{Γtot})
