...
lemma dCoord_sub_sub_regroup
  ...
  have hassoc :
    ...
    simpa [group_sub_add] using
      (group_sub_add (X r' θ') (Y r' θ') (Z r' θ')).symm
...
lemma flatten_comm_block_r
  ...
  - sumIdx (fun ρ => g M ρ b r θ *
                      sumIdx (fun lam => Γtot M r θ ρ Idx.r lam
                                           * Γtot M r θ lam Idx.θ a))
  -- commutator D_r (g on the left)
  - sumIdx (fun ρ => g M ρ b r θ *
                      sumIdx (fun lam => Γtot M r θ ρ Idx.r lam
                                           * Γtot M r θ lam Idx.θ b)) := by
  classical
  -- pointwise: distribute Γ over the two subtractions
  have hpt :
    ∀ d,
      Γtot M r θ d a Idx.r *
        ( dCoord Idx.θ (fun r θ => g M d b r θ) r θ
        ...
        - sumIdx (fun e => Γtot M r θ e Idx.θ b * g M d e r θ) )
      =
      Γtot M r θ d a Idx.r * dCoord Idx.θ (fun r θ => g M d b r θ) r θ
      - Γtot M r θ d a Idx.r * sumIdx (fun e => Γtot M r θ e Idx.θ d * g M e b r θ)
      - Γtot M r θ d a Idx.r * sumIdx (fun e => Γtot M r θ e Idx.θ b * g M d e r θ) := by
    intro d; ring
  ...
  have h₁ :
    ...
    - sumIdx (fun d => Γtot M r θ d a Idx.r *
                        sumIdx (fun e => Γtot M r θ e Idx.θ d * g M e b r θ))
    - sumIdx (fun d => Γtot M r θ d a Idx.r *
                        sumIdx (fun e => Γtot M r θ e Idx.θ b * g M d e r θ)) := by
    rw [← sumIdx_map_sub, ← sumIdx_map_sub]; apply sumIdx_congr; intro d; exact hpt d
  -- rearrange the 2nd term: pull Γ inside, swap sums, and put g on the left
  have h₂ :
    sumIdx (fun d => Γtot M r θ d a Idx.r *
                     sumIdx (fun e => Γtot M r θ e Idx.θ d * g M e b r θ))
    =
    ...
    -- Step 1: Pull Γ_d inside the inner sum
    conv_lhs => arg 1; intro d; rw [mul_sumIdx]
    -- Step 2: Swap sums
    rw [sumIdx_swap]
    -- Step 3: Rearrange and factor out g
    apply sumIdx_congr; intro e
    -- After swap: Σ_d (Γ_d * (Γ_e,d * g_e))
    -- Want: g_e * Σ_d (Γ_d * Γ_e,d)
    -- First rearrange inside the sum
    have heq : sumIdx (fun d => Γtot M r θ d a Idx.r * (Γtot M r θ e Idx.θ d * g M e b r θ))
             = sumIdx (fun d => (Γtot M r θ d a Idx.r * Γtot M r θ e Idx.θ d) * g M e b r θ) := by
      apply sumIdx_congr; intro d; ring
    rw [heq]
    -- First commute to get g on the left: (Γ * Γ) * g = g * (Γ * Γ)
    have hcomm : sumIdx (fun d => (Γtot M r θ d a Idx.r * Γtot M r θ e Idx.θ d) * g M e b r θ)
               = sumIdx (fun d => g M e b r θ * (Γtot M r θ d a Idx.r * Γtot M r θ e Idx.θ d)) := by
      apply sumIdx_congr; intro d; ring
    rw [hcomm]
...
/-- Flatten the θ-branch: Γ^ρ_{θ d}·(∂r g_{d b} − Σ Γ^e_{r d} g_{e b} − Σ Γ^e_{r b} g_{d e}) -/
lemma flatten_comm_block_θ
    (M r θ : ℝ) (a b : Idx) :
  sumIdx (fun d =>
    Γtot M r θ d a Idx.θ *
  ...
  - sumIdx (fun ρ => g M ρ b r θ *
                      sumIdx (fun lam => Γtot M r θ ρ Idx.θ lam
                                           * Γtot M r θ lam Idx.r a))
  - sumIdx (fun ρ => g M ρ b r θ *
                      sumIdx (fun lam => Γtot M r θ ρ Idx.θ lam
                                           * Γtot M r θ lam Idx.r b)) := by
  classical
  -- Identical algebra as flatten_comm_block_r, swapping r↔θ
  have hpt :
    ∀ d,
      Γtot M r θ d a Idx.θ *
        ( dCoord Idx.r (fun r θ => g M d b r θ) r θ
        ...
        - sumIdx (fun e => Γtot M r θ e Idx.r b * g M d e r θ) )
      =
      Γtot M r θ d a Idx.θ * dCoord Idx.r (fun r θ => g M d b r θ) r θ
      - Γtot M r θ d a Idx.θ * sumIdx (fun e => Γtot M r θ e Idx.r d * g M e b r θ)
      - Γtot M r θ d a Idx.θ * sumIdx (fun e => Γtot M r θ e Idx.r b * g M d e r θ) := by
    intro d; ring
  have h₁ :
    sumIdx (fun d =>
      Γtot M r θ d a Idx.θ *
    ...
    - sumIdx (fun d => Γtot M r θ d a Idx.θ *
                        sumIdx (fun e => Γtot M r θ e Idx.r d * g M e b r θ))
    - sumIdx (fun d => Γtot M r θ d a Idx.θ *
                        sumIdx (fun e => Γtot M r θ e Idx.r b * g M d e r θ)) := by
    rw [← sumIdx_map_sub, ← sumIdx_map_sub]; apply sumIdx_congr; intro d; exact hpt d
  have h₂ :
    sumIdx (fun d => Γtot M r θ d a Idx.θ *
                     sumIdx (fun e => Γtot M r θ e Idx.r d * g M e b r θ))
    ...
    rw [sumIdx_swap]
    apply sumIdx_congr; intro e
    have heq : sumIdx (fun d => Γtot M r θ d a Idx.θ * (Γtot M r θ e Idx.r d * g M e b r θ))
             = sumIdx (fun d => (Γtot M r θ d a Idx.θ * Γtot M r θ e Idx.r d) * g M e b r θ) := by
      apply sumIdx_congr; intro d; ring
    rw [heq]
    -- First commute to get g on the left
    have hcomm : sumIdx (fun d => (Γtot M r θ d a Idx.θ * Γtot M r θ e Idx.r d) * g M e b r θ)
               = sumIdx (fun d => g M e b r θ * (Γtot M r θ d a Idx.θ * Γtot M r θ e Idx.r d)) := by
      apply sumIdx_congr; intro d; ring
    rw [hcomm]
    -- Now factor out g using mul_sumIdx backward
    rw [← mul_sumIdx]
...
@[simp] lemma g_times_RiemannBody_comm
  ...
  have hsum :
    ...
    simpa using
      (sumIdx_map_sub
        (fun lam => Γtot M r θ k Idx.r lam * Γtot M r θ lam Idx.θ a)
        (fun lam => Γtot M r θ k Idx.θ lam * Γtot M r θ lam Idx.r a))
...
+ sumIdx (fun e =>
  ...
  have h_main :
    ...
    simpa using
      (sumIdx_map_sub
        (fun e => Γtot M r θ ρ μ a * Γtot M r θ e ν ρ * g M e b r θ)
        (fun e => Γtot M r θ ρ ν a * Γtot M r θ e μ ρ * g M e b r θ))
  have h_cross :
      (sumIdx (fun e =>
         Γtot M r θ ρ μ a * Γtot M r θ e ν b * g M ρ e r θ))
    - (sumIdx (fun e =>
    ...
    simpa using
      (sumIdx_map_sub
        (fun e => Γtot M r θ ρ μ a * Γtot M r θ e ν b * g M ρ e r θ)
        (fun e => Γtot M r θ ρ ν a * Γtot M r θ e μ b * g M ρ e r θ))
...
+ sumIdx (fun e =>
  ...
  have h_main :
    ...
    simpa using
      (sumIdx_map_sub
        (fun e => Γtot M r θ ρ μ b * Γtot M r θ e ν ρ * g M e a r θ)
        (fun e => Γtot M r θ ρ ν b * Γtot M r θ e μ ρ * g M e a r θ))
  have h_cross :
      (sumIdx (fun e =>
         Γtot M r θ ρ μ b * Γtot M r θ e ν a * g M ρ e r θ))
    - (sumIdx (fun e =>
    ...
    simpa using
      (sumIdx_map_sub
        (fun e => Γtot M r θ ρ μ b * Γtot M r θ e ν a * g M ρ e r θ)
        (fun e => Γtot M r θ ρ ν b * Γtot M r θ e μ a * g M ρ e r θ))
  -- 5) Rewrite those two blocks and finish with arithmetic.
  simp [h_main, h_cross]
  ring
...
lemma ΓΓ_quartet_split_b
  ...
  have first_block :
    ...
    classical
    -- Work per ρ, then reassemble with sumIdx_congr
    apply sumIdx_congr; intro ρ
    calc
      sumIdx (fun e =>
        ((Γtot M r θ ρ μ a * Γtot M r θ e ν ρ)
       - (Γtot M r θ ρ ν a * Γtot M r θ e μ ρ)) * g M e b r θ)
          = sumIdx (fun e =>
            ...
            - (Γtot M r θ ρ ν a * Γtot M r θ e μ ρ) * g M e b r θ) := by
                apply sumIdx_congr; intro e; ring
      _   = sumIdx (fun e => (Γtot M r θ ρ μ a * Γtot M r θ e ν ρ) * g M e b r θ)
          - sumIdx (fun e => (Γtot M r θ ρ ν a * Γtot M r θ e μ ρ) * g M e b r θ) :=
              sumIdx_map_sub _ _
      -- Now collapse each sum using sumIdx_reduce_by_diagonality_right
      _   = (Γtot M r θ ρ μ a * (g M b b r θ * Γtot M r θ b ν ρ))
          - (Γtot M r θ ρ ν a * (g M b b r θ * Γtot M r θ b μ ρ)) := by
                congr 1
                · calc
                    sumIdx (fun e => (Γtot M r θ ρ μ a * Γtot M r θ e ν ρ) * g M e b r θ)
                        = sumIdx (fun e => Γtot M r θ ρ μ a * (Γtot M r θ e ν ρ * g M e b r θ)) := by
                            apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ μ a * sumIdx (fun e => Γtot M r θ e ν ρ * g M e b r θ) :=
                            sumIdx_mul _ _
                    _   = Γtot M r θ ρ μ a * sumIdx (fun e => g M e b r θ * Γtot M r θ e ν ρ) := by
                            congr 1; apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ μ a * (g M b b r θ * Γtot M r θ b ν ρ) := by
                            rw [sumIdx_reduce_by_diagonality_right]
                · calc
                    sumIdx (fun e => (Γtot M r θ ρ ν a * Γtot M r θ e μ ρ) * g M e b r θ)
                        = sumIdx (fun e => Γtot M r θ ρ ν a * (Γtot M r θ e μ ρ * g M e b r θ)) := by
                            apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ ν a * sumIdx (fun e => Γtot M r θ e μ ρ * g M e b r θ) :=
                            sumIdx_mul _ _
                    _   = Γtot M r θ ρ ν a * sumIdx (fun e => g M e b r θ * Γtot M r θ e μ ρ) := by
                            congr 1; apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ ν a * (g M b b r θ * Γtot M r θ b μ ρ) := by
                            rw [sumIdx_reduce_by_diagonality_right]
      _   = g M b b r θ * (Γtot M r θ ρ μ a * Γtot M r θ b ν ρ)
          - g M b b r θ * (Γtot M r θ ρ ν a * Γtot M r θ b μ ρ) := by
                ring
      ...
      sumIdx (fun e =>
       ...
       - (Γtot M r θ ρ ν b * Γtot M r θ e μ ρ)) * g M e a r θ)
          = sumIdx (fun e =>
            ...
            - (Γtot M r θ ρ ν b * Γtot M r θ e μ ρ) * g M e a r θ) := by
                apply sumIdx_congr; intro e; ring
      _   = sumIdx (fun e => (Γtot M r θ ρ μ b * Γtot M r θ e ν ρ) * g M e a r θ)
          - sumIdx (fun e => (Γtot M r θ ρ ν b * Γtot M r θ e μ ρ) * g M e a r θ) :=
              sumIdx_map_sub _ _
      -- Now collapse each sum using sumIdx_reduce_by_diagonality_right
      _   = (Γtot M r θ ρ μ b * (g M a a r θ * Γtot M r θ a ν ρ))
          - (Γtot M r θ ρ ν b * (g M a a r θ * Γtot M r θ a μ ρ)) := by
                congr 1
                · calc
                    sumIdx (fun e => (Γtot M r θ ρ μ b * Γtot M r θ e ν ρ) * g M e a r θ)
                        = sumIdx (fun e => Γtot M r θ ρ μ b * (Γtot M r θ e ν ρ * g M e a r θ)) := by
                            apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ μ b * sumIdx (fun e => Γtot M r θ e ν ρ * g M e a r θ) :=
                            sumIdx_mul _ _
                    _   = Γtot M r θ ρ μ b * sumIdx (fun e => g M e a r θ * Γtot M r θ e ν ρ) := by
                            congr 1; apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ μ b * (g M a a r θ * Γtot M r θ a ν ρ) := by
                            rw [sumIdx_reduce_by_diagonality_right]
                · calc
                    sumIdx (fun e => (Γtot M r θ ρ ν b * Γtot M r θ e μ ρ) * g M e a r θ)
                        = sumIdx (fun e => Γtot M r θ ρ ν b * (Γtot M r θ e μ ρ * g M e a r θ)) := by
                            apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ ν b * sumIdx (fun e => Γtot M r θ e μ ρ * g M e a r θ) :=
                            sumIdx_mul _ _
                    _   = Γtot M r θ ρ ν b * sumIdx (fun e => g M e a r θ * Γtot M r θ e μ ρ) := by
                            congr 1; apply sumIdx_congr; intro e; ring
                    _   = Γtot M r θ ρ ν b * (g M a a r θ * Γtot M r θ a μ ρ) := by
                            rw [sumIdx_reduce_by_diagonality_right]
      _   = g M a a r θ * (Γtot M r θ ρ μ b * Γtot M r θ a ν ρ)
          - g M a a r θ * (Γtot M r θ ρ ν b * Γtot M r θ a μ ρ) := by
                ring
    ...
    have scalar_finish_bb :
        ...
        =
          ((- dCoord μ (fun r θ => Γtot M r θ b ν a) r θ
           ...
           + ( sumIdx (fun e => Γtot M r θ b μ e * Γtot M r θ e ν a)
             - sumIdx (fun e => Γtot M r θ b ν e * Γtot M r θ e μ a) )) * g M b b r θ := by
      classical
      -- Abbreviations (purely local, goal is unchanged):
      let gbb := g M b b r θ
      let A := dCoord μ (fun r θ => Γtot M r θ b ν a) r θ
      let B := dCoord ν (fun r θ => Γtot M r θ b μ a) r θ
    ...
    have scalar_finish_aa :
        ...
        =
          ((- dCoord μ (fun r θ => Γtot M r θ a ν b) r θ
           ...
           + ( sumIdx (fun e => Γtot M r θ a μ e * Γtot M r θ e ν b)
             - sumIdx (fun e => Γtot M r θ a ν e * Γtot M r θ e μ b) )) * g M a a r θ := by
      classical
      -- Abbreviations (purely local, goal is unchanged):
      let gaa := g M a a r θ
      let A := dCoord μ (fun r θ => Γtot M r θ a ν b) r θ
      let B := dCoord ν (fun r θ => Γtot M r θ a μ b) r θ
      let C := sumIdx (fun e => Γtot M r θ a μ e * Γtot M r θ e ν b)
...
lemma payload_cancel_a (M r θ : ℝ) (h_ext : Exterior M r θ) (μ ν a b : Idx) :
  ( sumIdx (fun ρ =>
      - (Γtot M r θ ρ ν a) * dCoord μ (fun r θ => g M ρ b r θ) r θ
      + (Γtot M r θ ρ μ a) * dCoord ν (fun r θ => g M ρ b r θ) r θ) )
+ ( sumIdx (fun ρ =>
...
lemma payload_cancel_b (M r θ : ℝ) (h_ext : Exterior M r θ) (μ ν a b : Idx) :
  ( sumIdx (fun ρ =>
      - (Γtot M r θ ρ ν b) * dCoord μ (fun r θ => g M a ρ r θ) r θ
      + (Γtot M r θ ρ μ b) * dCoord ν (fun r θ => g M a ρ r θ) r θ) )
+ ( sumIdx (fun ρ =>
...
lemma dGamma_match (M r θ : ℝ) (h_ext : Exterior M r θ) (μ ν a b : Idx) :
...
+ ( sumIdx (fun ρ => sumIdx (fun e =>
  ...
  calc
    ...
    _ = sumIdx (fun ρ => g M ρ ρ r θ * 0) := by
          apply sumIdx_congr; intro ρ; simp [hK0 ρ]
...
lemma ricci_identity_on_g_rθ_ext
  ...
  have hνμ :
    ...
    calc
      ...
      _   = 0 := by
          have hpt :
            (fun ρ => (Γtot M r θ ρ Idx.θ a) * 0 + (Γtot M r θ ρ Idx.θ b) * 0)
            = (fun _ => (0 : ℝ)) := by
            funext ρ; simp
          simp [hpt, sumIdx_zero]
...
lemma Riemann_swap_a_b_ext
  ...
  have hμν :
    ...
    calc
      ...
      _   = 0 := by
          have hpt :
            (fun ρ => (Γtot M r θ ρ μ a) * 0 + (Γtot M r θ ρ μ b) * 0)
            = (fun _ => (0 : ℝ)) := by
            funext ρ; simp
          simp [hpt, sumIdx_zero]
  ...
  have hνμ :
    ...
    calc
      ...
      _   = 0 := by
          have hpt :
            (fun ρ => (Γtot M r θ ρ ν a) * 0 + (Γtot M r θ ρ ν b) * 0)
            = (fun _ => (0 : ℝ)) := by
            funext ρ; simp
          simp [hpt, sumIdx_zero]
