...
lemma deriv_sq_id (r : ℝ) :
  ...
  have h0 : deriv (fun s => s * s) r = deriv id r * r + r * deriv id r :=
    deriv_mul differentiableAt_id differentiableAt_id
  have h : deriv (fun s => s * s) r = r + r := by
    simp [h0, deriv_id, one_mul]
  rw [show (fun s => s^2) = fun s => s * s by funext; simp [pow_two]]
  calc
    deriv (fun s => s * s) r = r + r := h
    _ = 2 * r := by ring
...
theorem Veff_null_deriv_zero_iff_exterior
  ...
  · -- ⇒
    ...
    -- Work with cleared form using minimal simpa:
...
theorem Gamma_r_φφ_from_LeviCivita
  ...
  have hD : deriv (fun s => g_φφ s θ) r = 2 * r * (Real.sin θ)^2 := by
    ...
    have h0 :
      ...
      simp [pow_two]
    have h1 :
        deriv (fun s => g_φφ s θ) r = (1 * r + r * 1) * (Real.sin θ)^2 := by
      simp [g_φφ, h0]
    calc
      deriv (fun s => g_φφ s θ) r
          = (1 * r + r * 1) * (Real.sin θ)^2 := h1
      _ = 2 * r * (Real.sin θ)^2 := by ring
...
@[simp] lemma Γtot_θ_θθ_zero (M r θ : ℝ) :
  Γtot M r θ Idx.θ Idx.θ Idx.θ = 0 := by simp [Γtot]

@[simp] lemma Γtot_t_θt_zero (M r θ : ℝ) :
  Γtot M r θ Idx.t Idx.θ Idx.t = 0 := by simp [Γtot]

@[simp] lemma Γtot_r_θr_zero (M r θ : ℝ) :
  Γtot M r θ Idx.r Idx.θ Idx.r = 0 := by simp [Γtot]
...
section DerivSimpHelpers
  ...
  @[simp] lemma deriv_neg_sin_mul_cos (θ : ℝ) :
    ...
    have hs := Real.hasDerivAt_sin θ
    have hc := Real.hasDerivAt_cos θ
    have hmul := (hs.mul hc).neg
    -- turn the HasDerivAt into a deriv identity
...
@[simp] lemma Ricci_rr_reduce (M r θ : ℝ) :
  ...
  simp only [Γtot]
  simp only [Γ_r_rr, Γ_t_tr, Γ_θ_rθ, Γ_φ_rφ]
  simp only [deriv_const]
  ring
...
/-
Freeze the radial Christoffels (and cot) for the whole section so `simp` never
unfolds them under a `deriv`. We will still expand *naked* occurrences explicitly.
We also freeze Γ_θ_φφ as it appears under a derivative in the R_φφ reduction.
-/
section RicciReductions
  ...
  /-- Canonical form for `R_{θθ}` (keep the radial derivative symbolic).
      CORRECTION: Added factor of 2 in the last term due to symmetry in the contraction (Term 4).
  -/
  @[simp] lemma Ricci_θθ_reduce (M r θ : ℝ) :
    ...
    simp only [Γtot]
    simp only [Γ_r_θθ, Γ_θ_rθ, Γ_φ_θφ, Γ_t_tr, Γ_r_rr, Γ_φ_rφ]
  ...
  /-- Canonical form for `R_{φφ}`.
      CORRECTION: Includes ∂_θ Γ^θ_{φφ} (Term 1, ρ=θ) and corrected contraction terms.
  -/
  @[simp] lemma Ricci_φφ_reduce (M r θ : ℝ) :
    ...
    -- Term 2 (dν=dφ) vanishes automatically as the metric is independent of φ.
    simp only [Γtot]
    simp only [Γ_r_φφ, Γ_φ_rφ, Γ_φ_θφ]
    ring
...
@[simp] lemma deriv_traceGamma_r
  ...
  have hderiv : deriv (fun s : ℝ => (2 : ℝ) / s) r = - (2 / r^2) := by
    calc
      deriv (fun s : ℝ => (2 : ℝ) / s) r
          = deriv (fun s : ℝ => (2 : ℝ) * s⁻¹) r := by
            simp [div_eq_mul_inv]
      _ = (2 : ℝ) * (-1 / r^2) := h2.deriv
      _ = - (2 / r^2) := by ring
...
lemma deriv_Γ_r_rr
  ...
  have h_sq' : HasDerivAt (fun s : ℝ => s * s) (r * 1 + 1 * r) r := by
    simpa using (hasDerivAt_id' (x := r)).mul (hasDerivAt_id' (x := r))
  ...
  -- 3) normalize the linear term and simplify the expression directly
  have hlin : (r * 1 + 1 * r) = 2 * r := by ring
  rw [hΓ_plus, hlin]
  -- Simplify algebraic structure: (-A)*(-B/C) = A*B/C
  simp only [neg_mul, mul_div_assoc]
...
lemma deriv_Γ_r_φφ (M r θ : ℝ) (_hr0 : r ≠ 0) :
  deriv (fun s => Γ_r_φφ M s θ) r
  ...
  have hfun :
    ...
    funext s; simp [Γ_r_φφ, Γ_r_θθ, mul_comm]
...
@[simp] lemma deriv_Γ_θ_φφ (θ : ℝ) :
  ...
  have hneg : deriv (fun t => -(Real.sin t * Real.cos t)) θ
    ...
    -- rewrite sin θ * (-sin θ) as -(sin θ * sin θ)
    have h := this
    simp [h, mul_neg, sub_eq_add_neg, add_comm]
  simp [pow_two, hneg]
...
theorem Ricci_θθ_vanishes
  ...
  classical
  -- hf0 is unused but kept for consistency with exterior_nonzeros structure.
  have ⟨hr0, _hf0, hr2M⟩ := exterior_nonzeros hM hr
  have hsθ : Real.sin θ ≠ 0 := sin_theta_ne_zero θ hθ
  
  -- Use the corrected reduction
  simp only [Ricci_θθ_reduce]
  -- Now substitute the derivative values.
  rw [deriv_Γ_r_θθ M r hr0, deriv_Γ_φ_θφ_cotangent θ hsθ]
  -- Expand and simplify
  simp [Γ_r_θθ, Γ_φ_θφ, Γ_t_tr, Γ_r_rr, Γ_θ_rθ, Γ_φ_rφ, f]
  -- field_simp needs hsθ because cotangent (Γ_φ_θφ) and its derivative (csc^2) are used.
  field_simp [hr0, hr2M, hsθ]

  -- Apply the trigonometric identity cos²θ = 1 - sin²θ
  have h_cos_sq : Real.cos θ ^ 2 = 1 - Real.sin θ ^ 2 := by
    have h1 : Real.sin θ ^ 2 + Real.cos θ ^ 2 = 1 := Real.sin_sq_add_cos_sq θ
    linarith
  simp only [h_cos_sq]
  
  -- The expression is now a polynomial identity solvable by ring.
  ring
...
theorem Ricci_φφ_vanishes
  ...
  -- Now substitute the derivative values
  rw [deriv_Γ_r_φφ M r θ hr0, deriv_Γ_θ_φφ θ]

  -- Substitute Γ_r_θθ derivative which appears inside deriv_Γ_r_φφ expansion
  rw [deriv_Γ_r_θθ M r hr0]

  -- Expand and simplify
  simp [Γ_r_φφ, Γ_φ_rφ, Γ_φ_θφ, Γ_θ_φφ, Γ_t_tr, Γ_r_rr, Γ_θ_rθ, Γ_φ_rφ, f]
  -- field_simp needs hsθ because Γ_φ_θφ involves division by sin θ.
  field_simp [hr0, hr2M, hsθ]
  ring

/-- All off-diagonal Ricci components vanish. -/
theorem Ricci_off_diagonal_vanish
    (M r θ : ℝ) (_hM : 0 < M) (_hr : 2*M < r) (μ ν : Idx) (h_ne : μ ≠ ν) :
  ...
  cases μ <;> cases ν <;> (simp [h_ne] at *; try (simp [Γtot]); try ring)
...
theorem TensorEngine_Height_Zero (S : Spacetime) :
  IsPinnedSchwarzschild S →
  ∃ (_engine : TensorEngine S), VacuumEFE S := by
  intro h_pin
