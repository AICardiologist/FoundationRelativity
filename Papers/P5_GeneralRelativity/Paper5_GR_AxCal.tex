...
\paragraph{Domain of validity: the exterior chart.}
We work on the open exterior chart $U = \{(t,r,\theta,\varphi) \mid r > 2M,\, 0 < \theta < \pi\}$ in Schwarzschild coordinates. On $U$, the metric components, Christoffel symbols, and hence the Riemann tensor are smooth; we compute all components $R^\rho{}_{\sigma\mu\nu}$ there. Points where these coordinates are singular---the event horizon $r = 2M$ and the coordinate poles $\theta \in \{0,\pi\}$---are outside $U$ and require either a second angular chart or a horizon-regular chart (e.g., Eddington--Finkelstein or Kruskal--Szekeres coordinates) if one wishes to make statements at those points. Our theorem is \emph{local} and makes no such global claims.

This scoped approach is standard mathematical practice: curvature is a local object, and differential geometry texts routinely compute $\Gamma$ and $R$ on an open set where coordinates are regular before addressing coordinate singularities or global structure separately~\cite{Wald1984,HawkingEllis1973,MTW1973}. Limiting our formalization to the exterior region is not a weakness but reflects the textbook workflow: establish results where the chart is valid, then---if the goal warrants it---extend via atlases, transition maps, or alternative coordinate systems. A theorem about an open set is a perfectly good theorem, and our Lean implementation mirrors this standard by explicitly bundling the domain hypothesis (\texttt{Exterior M r θ}) and proving all results under that predicate.
