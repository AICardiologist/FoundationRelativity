...
The key new abstraction is the classification of computational targets by their relationship to the Archimedean place.
...
The four engineering applications are organized by the descent structure of the Archimedean Principle:

\begin{itemize}[nosep]
\item \textbf{Projection descent} ($\LPO \to \BISH$): eliminates $\MP$.  This is the mechanism that makes Shor's algorithm work (algebraic targets localize under $\QFT$), $\LLL$ efficient (rational Gram--Schmidt), and algebraic-direct algorithms exact (integer row operations).
\item \textbf{Search descent} ($\LPO \to \BISH + \MP$): preserves $\MP$.  This is the obstruction that makes lattice problems hard (metric targets delocalize), $\BKZ$ slow (block-SVP search), and eigendecompose-round inexact ($\BDD$ search).
\end{itemize}
...
\toprule
\textbf{Theorem} & \textbf{Axioms} & \textbf{Load-bearing?} & \textbf{Notes} \\
\midrule
\texttt{archimedean\_security} & \texttt{ofReduceBool}, \texttt{trustCompiler} & Yes & \texttt{native\_decide} kernel axioms \\
\texttt{svp\_phase\_transition} & \texttt{ofReduceBool}, \texttt{trustCompiler} & Yes & Same \\
\texttt{conjugacy\_security\_ordering} & \texttt{ofReduceBool}, \texttt{trustCompiler} & Yes & Same \\
\texttt{eigendecomposition\_integrality} & \texttt{ofReduceBool}, \texttt{trustCompiler} & Yes & Same \\
\texttt{int\_sq\_sum\_one} & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} & Infra.\ only & Mathlib \texttt{Finset} machinery \\
\bottomrule
\end{tabular}
\end{center}
